...
class NetworkEnvironment:
    def __init__(self, topology="", num_total_channels=5,
                 ...
                 load_request_flag=False, load_request_pair_flag=False,
                 request_file_node_pair_name="requests/requests.pkl",
        ...
        self.get_request(request_file_name=request_file_node_pair_name, load_flag=load_request_pair_flag)
        ...
        self.DG_original = copy.deepcopy(self.DG)
        self.request_splits_dic = {}
        self.request_splits_modules_dic = {}
    ...
    def get_request(self, request_file_name="requests/requests.pkl", load_flag=False):
        ...
        neighbor_list = random.sample(edge_ug_list, len(edge_ug_list))
        # neighbor_list = random.sample(edge_ug_list, 1)
        # all_edges_list_num.extend(neighbor_list)
        neighbor_list.extend(all_edges_list_num)
        all_edges_list_num = neighbor_list
        # sorted_sample = [
        ...
        # for request in self.request_node_pair_num_list:
        #     if (request[0], request[1]) in list(self.DG.edges):
        #         print(request)
    ...
    def initialize_time_auxiliary_graph_ob_tr(self, DG_tmp:nx.DiGraph, start_time_slot_index=0, end_time_slot_index=0,
                                              qkp_remaining_keys_dict={}, reach_index=0, channel_index_selected=0,
                                              request_key_rate=0, storing=False):
        if DG_tmp is None:
            DG_tmp = self.DG
    ...
    def process_request(self):
        ...
        #                                         key=lambda item: self.request_key_rate_dic[item[0], item[1]], reverse=False)
    ...
    def time_driven_auxiliary_graph_shortest_path(self, time_slot, qkp_remaining_keys_dic,
        ...
        try:
            ...
            shortest_path_len_tmp = nx.path_weight(self.time_auxiliary_graph, shortest_path_nodes_list, "weight")
            # [1, '0-1-out', '0-2-in', 2, 3]
            try:
                len_tmp = nx.path_weight(self.time_auxiliary_graph, [1, '0-1-out', '0-2-in', 2, 3], "weight")
                len_tmp = nx.path_weight(self.time_auxiliary_graph, [1, '0-1-out'], "weight")
                len_tmp = nx.path_weight(self.time_auxiliary_graph, ['0-1-out', '0-2-in'], "weight")
                len_tmp = nx.path_weight(self.time_auxiliary_graph, ['0-2-in', 2], "weight")
                len_tmp = nx.path_weight(self.time_auxiliary_graph, [2, 3], "weight")

                len_tmp = nx.path_weight(self.time_auxiliary_graph, [1, '0-1-out', '0-3-in', 3], "weight")
                len_tmp = nx.path_weight(self.time_auxiliary_graph, [1, '0-1-out'], "weight")
                len_tmp = nx.path_weight(self.time_auxiliary_graph, ['0-1-out', '0-3-in'], "weight")
                len_tmp = nx.path_weight(self.time_auxiliary_graph, ['0-3-in', 3], "weight")
        ...
        if efficiency_first_run and \
                (num_quantum_channels + num_qkp_consumed_channels > threshold or num_quantum_channels > threshold) and \
                num_qkd_modules > self.request_splits_modules_dic[request[0], request[1]]:
                return False, -1, -1, -1, -1, -1, -1, -1
        else:
    ...
    def get_maximum_quantum_module_baseline(self, request):
        path_nodes_list = nx.shortest_path(self.DG, request[0], request[1], weight='length')
        return 2 * (len(path_nodes_list) - 1)

    def get_minimum_key_rate_baseline(self, request):
        path_nodes_list = nx.shortest_path(self.DG, request[0], request[1], weight='length')
        path_length = nx.path_weight(self.DG, path_nodes_list, weight='length')
        for index in range(len(reaches_list)):
            if path_length <= reaches_list[index]:
                return reaches_key_rate_list[index] / math.pow(loss_ob, len(path_nodes_list) - 2)
        return 0

    def get_num_splits_dic(self):
        for request_index in range(len(self.ordered_request_edge_list)):
            ...
            minimum_key_rate_baseline = self.get_minimum_key_rate_baseline(request)

            minimum_splits_baseline = math.ceil(request_key_rate_remaining / (maximum_key_rate_baseline / self.num_time_slots))
            maximum_splits_baseline = math.ceil(request_key_rate_remaining / (minimum_key_rate_baseline / self.num_time_slots))
            selected_splits = minimum_splits_baseline
            selected_splits_modules = np.inf
            for num_splits_ite in range(minimum_splits_baseline, maximum_splits_baseline+1):
                key_rate_ite = request_key_rate_remaining / num_splits_ite
                self.initialize_time_auxiliary_graph_ob_tr(DG_tmp=DG_tmp, start_time_slot_index=0,
                                                           end_time_slot_index=0,
                                                           qkp_remaining_keys_dict=qkp_remaining_keys_dic_tmp,
                                                           reach_index=0,
                                                           channel_index_selected=0,
                                                           request_key_rate=key_rate_ite,
                                                           storing=True)
                ...
                num_quantum_channels, num_physical_hops, num_virtual_hops, num_qkp_hops = \
                    self.time_driven_auxiliary_graph_shortest_path(time_slot=0,
                                                                   qkp_remaining_keys_dic=qkp_remaining_keys_dic_tmp,
                                                                   request=request,
                                                                   efficiency_first_run=False)
                if num_splits_ite * num_qkd_modules < selected_splits_modules:
                    selected_splits = num_splits_ite
                    selected_splits_modules = num_splits_ite * num_qkd_modules
                else:
                    break
            self.request_splits_dic[request[0], request[1]] = selected_splits
            self.request_splits_modules_dic[request[0], request[1]] = int(selected_splits_modules / selected_splits)


    def serve_ordered_request(self, k=3):
        self.get_num_splits_dic()
        for request_index in range(len(self.ordered_request_edge_list)):
            # *****We copy the DG_tmp and qkp_remaining_keys_dic_tmp. If the key is ok, we use it***************
            # https://stackoverflow.com/questions/39555831/how-do-i-copy-but-not-deepcopy-a-networkx-graph
            # todo: just to test
            ...
            #                                                storing=True)
            #     shortest_path_flag, shortest_path_nodes_list, generated_key_rate, num_qkd_modules, \
            #     num_quantum_channels, num_physical_hops, num_virtual_hops = \
            #         self.time_driven_auxiliary_graph_shortest_path(time_slot=0,
            #                                                        qkp_remaining_keys_dic=qkp_remaining_keys_dic_tmp,
            #                                                        request=request,
            #                                                        efficiency_first_run=True)
            ...
            #     else:
            #         break
            # selected_splits = self.request_splits_dic[request[0], request[1]]
            # tolerate_key_rate_baseline = request_key_rate_remaining / selected_splits
            num_splits = self.request_splits_dic[request[0], request[1]]
            key_rate_per_split = request_key_rate_remaining / num_splits

            # maximum_quantum_module_baseline = self.get_maximum_quantum_module_baseline(request)
            # tr_quantum_module_total = math.ceil(request_key_rate_remaining / (maximum_key_rate_baseline / self.num_time_slots))
            # tr_quantum_module_total = tr_quantum_module_total * maximum_quantum_module_baseline
            #
            # minimum_quantum_module_baseline = 2
            # ob_quantum_module_total = 0
            # if minimum_key_rate_baseline > 0:
            #     ob_quantum_module_total = math.ceil(request_key_rate_remaining / (minimum_key_rate_baseline / self.num_time_slots))
            # else:
            #     ob_quantum_module_total = np.Inf
            # ob_quantum_module_total = ob_quantum_module_total * minimum_quantum_module_baseline
            #
            # tolerate = 0.5
            #
            # tolerate_key_rate_baseline = 0.5 * (maximum_key_rate_baseline + minimum_key_rate_baseline)
            ...
            #     num_splits = math.ceil(request_key_rate_remaining / (tolerate_key_rate_baseline / self.num_time_slots))
            #     key_rate_per_split = request_key_rate_remaining / num_splits
            # else:
            #     continue
            ...
            # for time_slot in range(0, self.num_time_slots):
                ...
                selected_efficiency = {}
                # if num_splits == 1:
                if request_key_rate_remaining <= minimum_key_rate_baseline / self.num_time_slots:
                    time_slot_start = self.num_time_slots - 1
                else:
                    time_slot_start = 0
                for time_slot in range(time_slot_start, self.num_time_slots):
                # for time_slot in range(0, self.num_time_slots):
                # for channel_index in range(0, self.num_total_channels):
                    if request_key_rate_remaining <= 0:
                        break
                    ...
                    for reach_index in range(0, len(reaches_list)):
                        ...
                        # reach_key_rate < key_rate_per_split is the conservative way to say for only one split
                        ...
                        self.initialize_time_auxiliary_graph_ob_tr(DG_tmp=DG_tmp, start_time_slot_index=0,
                                                                   ...
                                                                   channel_index_selected=channel_index,
                                                                   request_key_rate=min(request_key_rate_remaining, key_rate_per_split))
                        shortest_path_flag, shortest_path_nodes_list, generated_key_rate, num_qkd_modules,\
                            num_quantum_channels, num_physical_hops, num_virtual_hops, num_qkp_hops = \
    ...
    def serve_ordered_request_old(self, k=3):
        for request_index in range(len(self.ordered_request_edge_list)):
            # *****We copy the DG_tmp and qkp_remaining_keys_dic_tmp. If the key is ok, we use it***************
            # https://stackoverflow.com/questions/39555831/how-do-i-copy-but-not-deepcopy-a-networkx-graph
            DG_tmp = copy.deepcopy(self.DG)
            ...
            for time_slot in range(0, self.num_time_slots):
                ...
                for channel_index in range(0, self.num_total_channels):
                    ...
                    selected_consumed_module = np.inf
                    for reach_index in range(0, len(reaches_list)):
                        path_flag, generated_key_rate, consumed_module = self.time_driven_shortest_path(time_slot, DG_tmp,
                                                qkp_remaining_keys_dic_tmp, request, request_key_rate_remaining, channel_index, reach_index,
                                                efficiency_first_run=True, unused_channel_dic=unused_channel_dic_tmp)
                        if path_flag:
                            if generated_key_rate / (consumed_module + 0.1) > selected_generated_key_rate / (selected_consumed_module + 0.1):
                                selected_reach_index = reach_index
                                selected_generated_key_rate = generated_key_rate
                                selected_consumed_module = consumed_module
                    if selected_reach_index != -1:
                        selected_reach_index_dic[channel_index] = selected_reach_index
                        selected_generated_key_rate_dic[channel_index] = selected_generated_key_rate
                        selected_consumed_module_dic[channel_index] = selected_consumed_module
                        ...
                        path_flag, generated_key_rate, consumed_module = self.time_driven_shortest_path(time_slot, DG_tmp,
                                    qkp_remaining_keys_dic_tmp, request, request_key_rate_remaining,
                                    channel_index, selected_reach_index, assign_flag=True, assign_key_rate=selected_generated_key_rate,
                                    efficiency_first_run=True, unused_channel_dic=unused_channel_dic_tmp)
                        if consumed_module > 0:
                            try:
                                unused_channel_dic_tmp[request[0], request[1], time_slot].remove(channel_index)
                            except Exception as e:
                                print(e)
    ...
    def serve_ordered_request_ob(self, k=3):
        for request_index in range(len(self.ordered_request_edge_list)):
            # *****We copy the DG_tmp and qkp_remaining_keys_dic_tmp. If the key is ok, we use it***************
            # https://stackoverflow.com/questions/39555831/how-do-i-copy-but-not-deepcopy-a-networkx-graph
            DG_tmp = copy.deepcopy(self.DG)
            ...
            for time_slot in range(0, self.num_time_slots):
                ...
                for channel_index in range(0, self.num_total_channels):
                    ...
                    for reach_index in range(0, len(reaches_list)):
                        path_flag, generated_key_rate, consumed_module = self.time_driven_shortest_path(time_slot, DG_tmp,
                                                qkp_remaining_keys_dic_tmp, request, request_key_rate_remaining, channel_index, reach_index,
                                                efficiency_first_run=False, unused_channel_dic=unused_channel_dic_tmp)
                        if path_flag:
                            if generated_key_rate / (consumed_module + 0.1) > selected_generated_key_rate / (selected_consumed_module + 0.1):
                                selected_reach_index = reach_index
                                selected_generated_key_rate = generated_key_rate
    ...
    def serve_ordered_request_partial(self, k=3):
        for request_index in range(len(self.ordered_request_edge_list)):
            # *****We copy the DG_tmp and qkp_remaining_keys_dic_tmp. If the key is ok, we use it***************
            # https://stackoverflow.com/questions/39555831/how-do-i-copy-but-not-deepcopy-a-networkx-graph
            DG_tmp = copy.deepcopy(self.DG)
                    ...
                    for reach_index in range(0, len(reaches_list)):
                        ...
                        if shortest_path_flag:
                            ...
                            qkp_remaining_keys_dic_tmp[node1, node2] = \
                                min(qkp_remaining_keys_dic_tmp[node1, node2] + generated_key_rate, self.num_max_qkp)
                ...
                for node1_index in range(0, len(nodes_list) - 1):
                    for node2_index in range(node1_index+1, len(nodes_list)):
                        ...
                        for channel_index in unused_channel_dic_tmp[node1, node2, time_slot]:
                            ...
                            path_for_node_pair_flag, path = self.check_available_channels_between_node_pair(
                                time_slot,
                                DG_tmp, [node1, node2], reach_index, channel_index_selected=channel_index)
                            if path_for_node_pair_flag:
                                pathNodes = self.pathNodesDict[path]
                                path_qkd_modules_available = True
                                for node_index in range(0, len(pathNodes)):
                                    node = pathNodes[node_index]
                                    if node_index == 0 or node_index == len(pathNodes) - 1:
                                        if DG_tmp.nodes[node]['qkd-modules-dic'][time_slot] < 1:
                                            path_qkd_modules_available = False
                                    else:
                                        if DG_tmp.nodes[node]['qkd-modules-dic'][time_slot] < 2:
                                            path_qkd_modules_available = False
                                if path_qkd_modules_available == False:
                                    continue

                                self.unused_channel_dic[node1, node2, time_slot].remove(channel_index)
                                # print("########################We are storing keys###############################")
                                generated_key_rate = reaches_key_rate_list[reach_index] / self.num_time_slots
                                generated_key_rate = generated_key_rate / math.pow(loss_ob, len(pathNodes) - 2)
                                # return reaches_key_rate_list[index] / math.pow(1.26, len(pathNodesList) - 2)
                                # update keys in QKP
                                qkp_remaining_keys_dic_tmp[node1, node2] = \
                                    min(qkp_remaining_keys_dic_tmp[node1, node2] + generated_key_rate, self.num_max_qkp)

                                # assign QKD node
                                # QKD module and channel assignment
                                DG_tmp.nodes[node1]['qkd-modules-dic'][time_slot] = DG_tmp.nodes[node1]['qkd-modules-dic'][time_slot] - 1
                                DG_tmp.nodes[node2]['qkd-modules-dic'][time_slot] = DG_tmp.nodes[node2]['qkd-modules-dic'][time_slot] - 1

                                # assign quantum channel
                                pathEdges = self.pathEdgesDict[path]
                                # self.DG.edges[edge]['channels'] = [0 for i in range(self.num_total_channels)]
                                # self.DG.edges[edge]['channels-dic'][i] = [0 for i in range(self.num_total_channels)]
                                for edge in pathEdges:
                                    if DG_tmp.edges[edge]['channels-dic'][time_slot][channel_index] == 1:
                                        print(
                                            "***************************Error in channel assignment******************")
                                    else:
                                        DG_tmp.edges[edge]['channels-dic'][time_slot][channel_index] = 1
                            self.DG = DG_tmp
                            self.qkp_remaining_keys_dic = qkp_remaining_keys_dic_tmp
        ...
        if path:
            ...
            for index in range(len(reaches_list)):
                if pathLength <= reaches_list[index]:
                    return reaches_key_rate_list[index] / math.pow(loss_ob, len(pathNodesList) - 2)
            return 0
        else:
            return 0
    ...
    def time_driven_shortest_path(self, time_slot, DG_tmp:nx.DiGraph, qkp_remaining_keys_dic,
                                  request, request_key_rate, channel_index, reach_index, assign_flag=False,
                                  assign_key_rate=1, efficiency_first_run=False, unused_channel_dic=None):
        """
        :parma time_slot: the current time to schedule
        :return: the shortest path, the achieved key rate, number of QKD modules consumed
        """
        # if request[0] == 2 and request[1] == 5 and channel_index == 0 and reach_index == 0:
        #     print("Error")
        visited = set()
        unvisited = set(DG_tmp.nodes)
        u = request[0]
        v = request[1]

        distances = {u: 0}
        # distance for the node if it is not visited with QKD modules
        distances_no_module = {u: 0}
        t_path_previous_dic = {}
        t_path_previous_no_module_dic = {}
        path_current_dic = {}
        visited_with_module_dic = {}
        key_ele_dic = {}
        key_ele_no_module_dic = {}

        # each key in the dic is a list [predecessor node, flag of using QKD modules]
        pred_dic = {}
        pred_no_module_dic = {}
        ...
        for node in unvisited:
            if node == u:
                ...
                distances_no_module[node] = np.inf
                t_path_previous_dic[node] = [-1, -1]
                t_path_previous_no_module_dic[node] = [-1, -1]
                visited_with_module_dic[node] = False
                key_ele_dic[node] = [0, 0, 0]
                key_ele_no_module_dic[node] = [0, 0, 0]

        cur_node = u
        # True if we need qQKD module to reach current node
        hFlag = False
        hPreviousFlag = False
        hPreviousTimeslot = -1
        distanceCur = 0
        ...
        while len(unvisited) > 0:
            # here we found the cur_node
            if cur_node == v:
                break
            # Pull up neighbors
            # neighbors = DG_tmp[cur_node]
            neighbors = []
            for node in unvisited:
                if node != cur_node:
            ...
            for node in neighbors:
                ...
                # check if the length is a possible solution
                if (distanceCur + 1) > distances[node] and (distanceCur + 1) > distances_no_module[node]:
                    continue
                key_ele_tmp = [0, 0, 0]
                node1 = min(cur_node, node)
                node2 = max(cur_node, node)
                try:
                    if qkp_remaining_keys_dic[node1, node2] > 0.01:
                        key_ele_tmp[2] = min(qkp_remaining_keys_dic[node1, node2], request_key_rate)
                except Exception as e:
                    print("Error")

                t_previous_tmp = -1
                path_previous_tmp = -1
                if key_ele_tmp[2] < reaches_key_rate_list[reach_index] / self.num_time_slots:
                    for time_slot_previous in range(0, time_slot):
                        # todo: modify the unused channel
                        if unused_channel_dic:
                            node1 = min(cur_node, node)
                            node2 = max(cur_node, node)
                            # print(unused_channel_dic)
                            # print(channel_index)
                            if channel_index not in unused_channel_dic[node1, node2, time_slot_previous]:
                                continue
                        if DG_tmp.nodes[cur_node]['qkd-modules-dic'][time_slot_previous] == 0 or DG_tmp.nodes[node]['qkd-modules-dic'][time_slot_previous] == 0:
                            continue
                        elif DG_tmp.nodes[cur_node]['qkd-modules-dic'][time_slot_previous] == 1 \
                                and hPreviousTimeslot == time_slot_previous and hPreviousFlag:
                            continue
                        # check each previous time-slot
                        path_for_node_pair_flag, path = self.check_available_channels_between_node_pair(time_slot_previous,
                                                            DG_tmp, [cur_node, node], reach_index,
                                                            channel_index_selected=channel_index,
                                                            efficiency_first_run=efficiency_first_run)
                        if path_for_node_pair_flag:
                            #key_ele_tmp[1] = reaches_key_rate_list[reach_index] / self.num_time_slots
                            key_ele_tmp[1] = self.get_key_rate_of_path(path) / self.num_time_slots
                            t_previous_tmp = time_slot_previous
                            path_previous_tmp = path
                            break

                if path_previous_tmp != -1:
                    cost_edge_previous = 1
                else:
                    cost_edge_previous = 0
                if sum(key_ele_tmp) >= reaches_key_rate_list[reach_index] / self.num_time_slots:
                    # self.DG.nodes[node]['qkd-modules-dic'][i] = self.num_qkd_modules
                    if DG_tmp.nodes[node]['qkd-modules-dic'][time_slot] > 1 and distanceCur + cost_edge_previous < distances[node]:
                        distances[node] = distanceCur + cost_edge_previous
                        key_ele_dic[node] = key_ele_tmp
                        t_path_previous_dic[node] = [t_previous_tmp, path_previous_tmp]
                        pred_dic[node] = [cur_node, hFlag]
                    elif DG_tmp.nodes[node]['qkd-modules-dic'][time_slot] <= 1 \
                            and distanceCur + cost_edge_previous < distances_no_module[node]:
                        distances_no_module[node] = distanceCur + cost_edge_previous
                        key_ele_no_module_dic[node] = key_ele_tmp
                        t_path_previous_no_module_dic[node] = [t_previous_tmp, path_previous_tmp]
                        pred_no_module_dic[node] = [cur_node, hFlag]
                else:
                    if distanceCur + 1 >= distances[node]:
                        continue
                    elif DG_tmp.nodes[cur_node]['qkd-modules-dic'][time_slot] == 0 or DG_tmp.nodes[node]['qkd-modules-dic'][time_slot] == 0:
                        continue
                    elif DG_tmp.nodes[cur_node]['qkd-modules-dic'][time_slot] == 1 and hFlag == True:
                        continue
                    else:
                        path_for_node_pair_flag, path = self.check_available_channels_between_node_pair(time_slot,
                                                                DG_tmp, [cur_node, node], reach_index,
                                                                channel_index_selected=channel_index, efficiency_first_run=efficiency_first_run)
                        if path_for_node_pair_flag:
                            # todo: change the key rate here to the maximum capability of it
                            #key_ele_tmp[0] = reaches_key_rate_list[reach_index] / self.num_time_slots
                            key_ele_tmp[0] = self.get_key_rate_of_path(path) / self.num_time_slots
                            path_current_dic[node] = path

                            distances[node] = distanceCur + 1
                            key_ele_dic[node] = key_ele_tmp
                            pred_dic[node] = [cur_node, hFlag]

            selected_node = -1
            selected_cost = np.inf
            # True if the selected node does not need to use a QKD module
            selected_no_module_flag = False
            selected_no_module_previous_flag = False
            selected_previous_timeslot = -1

            if selected_node == u:
                print("Error")
            for node in neighbors:
                cost_tmp = min(distances[node], distances_no_module[node])

                if cost_tmp < selected_cost:
                    selected_node = node
                    selected_cost = cost_tmp
                    if selected_cost == distances_no_module[node]:
                        if key_ele_no_module_dic[node][0]:
                            selected_no_module_flag = False
                        else:
                            selected_no_module_flag = True
                        if key_ele_no_module_dic[node][1]:
                            selected_no_module_previous_flag = False
                            selected_previous_timeslot = t_path_previous_no_module_dic[node][0]
                        else:
                            selected_no_module_previous_flag = True
                    else:
                        if key_ele_dic[node][0]:
                            selected_no_module_flag = False
                        else:
                            selected_no_module_flag = True
                        if key_ele_dic[node][1]:
                            selected_no_module_previous_flag = False
                            selected_previous_timeslot = t_path_previous_dic[node][0]
                        else:
                            selected_no_module_previous_flag = True
            ...
            if selected_cost < np.inf:
                ...
                hFlag = not selected_no_module_flag
                hPreviousFlag = not selected_no_module_previous_flag
                hPreviousTimeslot = selected_previous_timeslot
                distanceCur = selected_cost
            else:
                successFlag = False
                break
        ...
        consumed_module = 1
        generated_key_rate = 1
        if not successFlag:
            return False, generated_key_rate, consumed_module
        else:
            # get path
            cur_node = v
            # [predecessor node, flag of using QKD modules]
            try:
                if distances_no_module[cur_node] <= distances[cur_node]:
                    #[pred_node, hFlag] = pred_no_module_dic[cur_node]
                    hFlag = False
                else:
                    # [pred_node, hFlag] = pred_dic[cur_node]
                    if key_ele_dic[cur_node][0] > 0:
                        hFlag = True
                    else:
                        hFlag = False
            except Exception as e:
            ...
            path_edges_list = []
            path_key_elem_list = []
            path_t_path_previous_list = []
            path_current_path_list = []
            while cur_node != u:
                path_nodes_list.append(cur_node)
                # we do not need QKD modules to reach current node
                if not hFlag and DG_tmp.nodes[cur_node]['qkd-modules-dic'][time_slot] <= 1:
                    [pred_node, hFlag] = pred_no_module_dic[cur_node]
                    path_key_elem_list.append(key_ele_no_module_dic[cur_node])
                    path_t_path_previous_list.append(t_path_previous_no_module_dic[cur_node])
                    path_current_path_list.append(None)
                else:
                    [pred_node, hFlag] = pred_dic[cur_node]
                    path_key_elem_list.append(key_ele_dic[cur_node])
                    path_t_path_previous_list.append(t_path_previous_dic[cur_node])
                    if key_ele_dic[cur_node][0] > 0:
                        path_current_path_list.append(path_current_dic[cur_node])
                    else:
                        path_current_path_list.append(None)
                cur_node = pred_node
            ...
            path_nodes_list.append(cur_node)
            # assign QKP keys
            path_key_sum_list = [sum(elem) for elem in path_key_elem_list]
            path_usage_qkd_module_list = [1 if elem[0] > 0 else 0 for elem in path_key_elem_list]
            path_usage_qkd_module_all_slots_list = [1 if elem[0] > 0 or elem[1] > 0 else 0 for elem in path_key_elem_list]
            generated_key_rate = min(path_key_sum_list)
            consumed_module = 2 * sum(path_usage_qkd_module_all_slots_list)

            if efficiency_first_run:
                num_edges_path = 0
                for index in range(0, len(path_nodes_list)-1):
                    if path_usage_qkd_module_list[index]:
                        path = path_current_path_list[index]
                        pathEdges = self.pathEdgesDict[path]
                        num_edges_path += len(pathEdges)
                    if path_key_elem_list[index][1] > 0:
                        # QKD module and channel assignment
                        path = path_t_path_previous_list[index][1]
                        pathEdges = self.pathEdgesDict[path]
                        num_edges_path += len(pathEdges)
                # num_edges_path = len(path_nodes_list) - 1
                if num_edges_path > self.nodePairsPathThresholdNumEdgesDic[u, v]:
                    return False, -1, -1

            if assign_flag:
                for index in range(0, len(path_nodes_list)-1):
                    node1 = min(path_nodes_list[index], path_nodes_list[index + 1])
                    node2 = max(path_nodes_list[index], path_nodes_list[index + 1])
                    # if path_key_sum_list[index] > request_key_rate:
                    #     qkp_remaining_keys_dic[node1, node2] = min(path_key_sum_list[index] - request_key_rate, self.num_max_qkp)
                    qkp_remaining_keys_dic[node1, node2] -= path_key_elem_list[index][2]
                    if assign_key_rate > generated_key_rate:
                        print("************************Error**************************")
                    qkp_remaining_keys_dic[node1, node2] += path_key_sum_list[index] - assign_key_rate
                    qkp_remaining_keys_dic[node1, node2] = min(qkp_remaining_keys_dic[node1, node2], self.num_max_qkp)
                    if path_usage_qkd_module_list[index]:
                        # QKD module and channel assignment
                        DG_tmp.nodes[node1]['qkd-modules-dic'][time_slot] = DG_tmp.nodes[node1]['qkd-modules-dic'][
                                                                                time_slot] - 1
                        DG_tmp.nodes[node2]['qkd-modules-dic'][time_slot] = DG_tmp.nodes[node2]['qkd-modules-dic'][
                                                                            time_slot] - 1
                        path = path_current_path_list[index]
                        pathEdges = self.pathEdgesDict[path]
                        # self.DG.edges[edge]['channels'] = [0 for i in range(self.num_total_channels)]
                        ...
                        for edge in pathEdges:
                            if DG_tmp.edges[edge]['channels-dic'][time_slot][channel_index] == 1:
                                ...
                                DG_tmp.edges[edge]['channels-dic'][time_slot][channel_index] = 1
                    if path_key_elem_list[index][1] > 0:
                        print("*********************Now we are using the qkd modules in previous time-slot*****************************")
                        time_slot_previous = path_t_path_previous_list[index][0]
                        DG_tmp.nodes[node1]['qkd-modules-dic'][time_slot_previous] = DG_tmp.nodes[node1]['qkd-modules-dic'][
                                                                                time_slot_previous] - 1
                        DG_tmp.nodes[node2]['qkd-modules-dic'][time_slot_previous] = DG_tmp.nodes[node2]['qkd-modules-dic'][
                                                                                time_slot_previous] - 1
                        # todo: channel for the the previous path
                        if unused_channel_dic:
                            unused_channel_dic[node1, node2, time_slot_previous].remove(channel_index)
                        ...
                        # self.DG.edges[edge]['channels-dic'][i] = [0 for i in range(self.num_total_channels)]
                        for edge in pathEdges:
                            if DG_tmp.edges[edge]['channels-dic'][time_slot_previous][channel_index] == 1:
                                print("********************************Error in channel assignment")
                            else:
                                DG_tmp.edges[edge]['channels-dic'][time_slot_previous][channel_index] = 1

            return True, generated_key_rate, consumed_module
