...
class NetworkEnvironment:
    def __init__(self, topology="", num_total_channels=5,
        ...
        self.request_splits_modules_upper_dic = {}
    ...
    def get_request(self, request_file_name="requests/requests.pkl", load_flag=False):
        ...
        if self.request_percentage < 1:
            ...
            self.request_node_pair_num_list = [[1,2], [2,3], [3,4], [4,5], [1,5], [1,4], [1,3], [3,5]]
    ...
    def assign_request_keyrate(self, keyRate=12, multiBitrateFlag=False, unidirectionalFlag=True,
        ...
        # edge_ug_list = [[item[0], item[1]] if item[0] < item[1] else [item[1], item[0]] for item in edge_ug_list]
        # edge_other_list = []
        # for edge in all_edges_list_num:
        #     if edge not in edge_ug_list:
        #         edge_other_list.append(edge)
        # for nodePair in self.request_key_rate_dic.keys():
        #     if [nodePair[0], nodePair[1]] in edge_ug_list:
        #         self.request_key_rate_dic[nodePair[0], nodePair[1]] = 9
        #     else:
        #         self.request_key_rate_dic[nodePair[0], nodePair[1]] = 12
        rand_value = np.random.randint(-10, 10) * 0.01
        for nodePair in self.request_key_rate_dic.keys():
            self.request_key_rate_dic[nodePair[0], nodePair[1]] += rand_value
            #self.request_key_rate_dic[nodePair[0], nodePair[1]] += np.random.uniform(-0.1, 0.1)
    ...
    def assign_qkp_storing_keys(self, qkp_key_rate=12, multi_key_rate_flag=False,
        ...
        for edge in all_edges_list_num:
            if edge not in edge_ug_list:
                edge_other_list.append(edge)
        for nodePair in self.qkp_remaining_keys_dic.keys():
            if [nodePair[0], nodePair[1]] in edge_ug_list:
                self.qkp_remaining_keys_dic[nodePair[0], nodePair[1]] = 3#2.5
            else:
                self.qkp_remaining_keys_dic[nodePair[0], nodePair[1]] = 0
    ...
    def process_request(self):
        ...
        self.ordered_request_edge_list = sorted(self.request_node_pair_num_list,
                                                key=lambda item: (self.get_shortest_path_length(item),
                                                                  -self.request_key_rate_remaining_dic[
                                                                      item[0], item[1]]), reverse=False)
    ...
    def time_driven_auxiliary_graph_shortest_path(self, time_slot, qkp_remaining_keys_dic,
        ...
        if efficiency_first_run and \
                ...
                and (num_qkd_modules > self.request_splits_modules_dic[request[0], request[1]]):
                #num_qkd_modules > 2 * num_quantum_channels:
                #num_qkd_modules > self.request_splits_modules_dic[request[0], request[1]]:
            return False, -1, -1, -1, -1, -1, -1, -1
        # if efficiency_first_run and num_qkp_consumed_channels > threshold:
        # # if efficiency_first_run and num_qkd_modules > \
        # #         min(self.request_splits_modules_dic[request[0], request[1]]+1, threshold*2):
        #     return False, -1, -1, -1, -1, -1, -1, -1
        else:
    ...
    def assign_shortest_path_auxiliary_graph(self, shortest_path_nodes_list, DG_tmp, assign_key_rate,
        ...
        if len(shortest_path_nodes_list) == 0:
            return
        request_node_start = shortest_path_nodes_list[0]
        request_node_end = shortest_path_nodes_list[-1]
        _, _, node_int1_start = self.get_information_node_auxiliary_graph(request_node_start)
        _, _, node_int1_end = self.get_information_node_auxiliary_graph(request_node_end)
        request_key_rate_achievable = np.inf
        ...
        for node_index in range(len(shortest_path_nodes_list)-1):
            ...
            if node_int1 != node_int2:
                ...
                if quantum_node_flag1 and quantum_node_flag2:
                    ...
                    if self.time_auxiliary_graph.edges[node1, node2]["keyrate"] < request_key_rate_achievable:
                        request_key_rate_achievable = self.time_auxiliary_graph.edges[node1, node2]["keyrate"]
                    # todo: do not save
                ...
                # if we only use the QKP
                    ...
                    request_key_rate_achievable = assign_key_rate
                    pass
                else:
                    raise Exception("Error: unexpected links between two nodes")
        ...
        # todo: save only for the pair
        qkp_pair_storing_maximum = \
            self.get_qkp_pair_storing_maximum([request_node_start, request_node_end], qkp_remaining_keys_dic)
        qkp_pair_storing_achievable = qkp_remaining_keys_dic[request_node_start, request_node_end] + \
                                      request_key_rate_achievable - assign_key_rate
        qkp_remaining_keys_dic[request_node_start, request_node_end] = \
            min(qkp_pair_storing_maximum, qkp_pair_storing_achievable)
    ...
    def get_num_splits_request(self, request, request_key_rate_remaining):
        DG_tmp = copy.deepcopy(self.DG_original)
        qkp_remaining_keys_dic_tmp = copy.deepcopy(self.qkp_remaining_keys_dic)
        for key in qkp_remaining_keys_dic_tmp:
            qkp_remaining_keys_dic_tmp[key] = 0
        ...
        minimum_splits_baseline = math.ceil(
            request_key_rate_remaining / (maximum_key_rate_baseline / self.num_time_slots))
        maximum_splits_baseline = math.ceil(
            request_key_rate_remaining / (minimum_key_rate_baseline / self.num_time_slots))
    ...
    def get_num_splits_dic(self):
        for request_index in range(len(self.ordered_request_edge_list)):
            ...
            qkp_remaining_keys_dic_tmp = copy.deepcopy(self.qkp_remaining_keys_dic)
            for key in qkp_remaining_keys_dic_tmp.keys():
                qkp_remaining_keys_dic_tmp[key] = 0
            ...
            if request_key_rate_remaining <= 0.01:
                self.request_splits_dic[request[0], request[1]] = 0
                self.request_splits_modules_dic[request[0], request[1]] = 0
                continue
            maximum_key_rate_baseline = self.get_maximum_key_rate_baseline(request)
            minimum_key_rate_baseline = self.get_minimum_key_rate_baseline(request)
            ...
            for num_splits_ite in range(minimum_splits_baseline, maximum_splits_baseline+1):
                key_rate_ite = request_key_rate_remaining / num_splits_ite
                key_rate_ite_int = math.ceil(key_rate_ite)
                self.initialize_time_auxiliary_graph_ob_tr(DG_tmp=DG_tmp, start_time_slot_index=0,
                                                           end_time_slot_index=0,
                                                           qkp_remaining_keys_dict=qkp_remaining_keys_dic_tmp,
                                                           reach_index=0,
                ...
                num_quantum_channels, num_physical_hops, num_virtual_hops, num_qkp_hops = \
                    self.time_driven_auxiliary_graph_shortest_path(time_slot=0,
                                                                   ...
                                                                   request=request,
                                                                   efficiency_first_run=False)
                num_splits_ite_int = math.ceil(request_key_rate_remaining / key_rate_ite_int)
                if num_splits_ite_int * num_qkd_modules < selected_splits_modules:
                    selected_splits = num_splits_ite_int
                    selected_splits_modules = num_splits_ite_int * num_qkd_modules
                else:
                    break
    ...
    def serve_ordered_request(self, k=3):
        ...
        for request_index in range(len(self.ordered_request_edge_list)):
            ...
            key_rate_per_split = math.ceil(key_rate_per_split)

            # todo: whether to change it
            # if qkp_remaining_keys_dic_tmp[request[0], request[1]] >= key_rate_per_split:
            #     # assign_key_rate_num_split_tmp = \
            #     #     min(num_splits, math.ceil(qkp_remaining_keys_dic_tmp[request[0], request[1]] / key_rate_per_split))
            #     # assign_key_rate_tmp = key_rate_per_split * assign_key_rate_num_split_tmp
            #     # qkp_remaining_keys_dic_tmp[request[0], request[1]] -= assign_key_rate_tmp
            #     # request_key_rate_remaining -= assign_key_rate_tmp
            #     assign_key_rate_tmp = \
            #         min(qkp_remaining_keys_dic_tmp[request[0], request[1]], request_key_rate_remaining)
            #     qkp_remaining_keys_dic_tmp[request[0], request[1]] -= assign_key_rate_tmp
            #     request_key_rate_remaining -= assign_key_rate_tmp
            #     if request_key_rate_remaining < 0.01:
            #         continue
            #     num_splits = self.get_num_splits_request(request, request_key_rate_remaining)
            #     key_rate_per_split = request_key_rate_remaining / num_splits
            ...
            # for time_slot in range(0, self.num_time_slots):
                ...
                # for channel_index in range(0, self.num_total_channels):
                    ...
                    if selected_reach_index != -1:
                        ...
                        request_key_rate_remaining = math.ceil(request_key_rate_remaining - selected_generated_key_rate)
        ...
        for request_index in range(len(ordered_node_pair_num_list)):
            ...
            for time_slot in range(0, self.num_time_slots):
                ...
                for channel_index in unused_channel_dic_tmp[node1, node2, time_slot]:
                    ...
                    num_quantum_channels_selected = np.inf
                    num_generated_key_rate_selected = 0
                    num_qkd_modules_selected = np.inf
                    num_reach_index_selected = -1
                    shortest_path_nodes_list_selected = []
                    for reach_index in range(0, len(reaches_list)):
                        ...
                        num_quantum_channels, num_physical_hops, num_virtual_hops, num_qkp_hops = \
                            self.time_driven_auxiliary_graph_shortest_path(time_slot=time_slot,
                                                                           ...
                                                                           request=request,
                                                                           efficiency_first_run=False)
                        if shortest_path_flag and num_quantum_channels <= num_quantum_channels_selected \
                                and generated_key_rate / num_qkd_modules > num_generated_key_rate_selected / num_qkd_modules_selected:
                            num_quantum_channels_selected = num_quantum_channels
                            num_generated_key_rate_selected = generated_key_rate
                            num_qkd_modules_selected = num_qkd_modules
                            num_reach_index_selected = reach_index
                            shortest_path_nodes_list_selected = shortest_path_nodes_list
                            break

                    # if num_generated_key_rate_selected < 2.8:
                    #     continue

                    self.assign_shortest_path_auxiliary_graph(
                        shortest_path_nodes_list=shortest_path_nodes_list_selected,
                        DG_tmp=DG_tmp, assign_key_rate=num_generated_key_rate_selected,
                        qkp_remaining_keys_dic=qkp_remaining_keys_dic_tmp,
                        channel_index=channel_index)

                    if num_qkd_modules_selected > 0 and num_reach_index_selected >= 0:
                        try:
                            unused_channel_dic_tmp[request[0], request[1], time_slot].remove(channel_index)
                        except Exception as e:
                            print(e)
                    ...
                    qkp_remaining_keys_dic_tmp[node1, node2] = \
                        min(qkp_remaining_keys_dic_tmp[node1, node2] + num_generated_key_rate_selected, qkp_pair_storing_maximum)
