...
class NetworkEnvironment:
                ...
                for i2 in range(i1+1, len(nodes_list)):
                    ...
                    if (node1, node2) in qkp_remaining_keys_dict.keys():
                        if qkp_remaining_keys_dict[node1, node2] >= reach_key_rate / self.num_time_slots or \
                            ...
                            self.time_auxiliary_graph.add_edge(node1, node2, weight=3*6E6+6E-6*num_edges_tmp, keyrate=edge_key_rate)
                            self.time_auxiliary_graph.add_edge(node2, node1, weight=3*6E6+6E-6*num_edges_tmp, keyrate=edge_key_rate)
            ...
            for node in nodes_list:
                if DG_tmp.nodes[node]['qkd-modules-dic'][time_slot_index]:
                    ...
                    if time_slot_index == end_time_slot_index:
                        if DG_tmp.nodes[node]['qkd-modules-dic'][time_slot_index] > 1:
                            self.time_auxiliary_graph.add_edge(node, out_node_name, weight=6E6 + 0.5)
                            self.time_auxiliary_graph.add_edge(in_node_name, node, weight=6E6 + 0.5)
                            #self.time_auxiliary_graph.add_edge(node, out_node_name, weight=1)
                        else:
                            #self.time_auxiliary_graph.add_edge(node, out_node_name, weight=2.5)
                            self.time_auxiliary_graph.add_edge(node, out_node_name, weight=6E6 + 1.25)
                            self.time_auxiliary_graph.add_edge(in_node_name, node, weight=6E6 + 1.25)
                    else:
                        if DG_tmp.nodes[node]['qkd-modules-dic'][time_slot_index] > 1:
                            self.time_auxiliary_graph.add_edge(node, out_node_name, weight=6E6 + 0.5 + 6E-9)
                            self.time_auxiliary_graph.add_edge(in_node_name, node, weight=6E6 + 0.5 + 6E-9)
                            #self.time_auxiliary_graph.add_edge(node, out_node_name, weight=1)
                        else:
                            #self.time_auxiliary_graph.add_edge(node, out_node_name, weight=2.5)
                            self.time_auxiliary_graph.add_edge(node, out_node_name, weight=6E6 + 1.25 + 6E-9)
                            self.time_auxiliary_graph.add_edge(in_node_name, node, weight=6E6 + 1.25 + 6E-9)
            ...
            for i1 in range(len(activated_nodes_list)-1):
                for i2 in range(i1+1, len(activated_nodes_list)):
                    ...
                    if flag:
                        ...
                        self.time_auxiliary_graph.add_edge(out_node1_name, in_node2_name, weight=6E6+edge_weight+num_edges*6E-6,
                    ...
                    if flag:
                        ...
                        self.time_auxiliary_graph.add_edge(out_node2_name, in_node1_name,
                                                           weight=6E6+edge_weight + num_edges * 6E-6,
                                                           path=path_selected,
    ...
    def output_stored_keys(self):
        ...
        print("############## Key stored all is: ", key_stored_all)
        ...
        self.ordered_request_edge_list = sorted(self.request_node_pair_num_list,
                                                key=lambda item: (
                                                                  self.request_key_rate_remaining_dic[
                                                                      item[0], item[1]], self.get_shortest_path_length(item)), reverse=False)
    ...
    def time_driven_auxiliary_graph_shortest_path(self, time_slot, qkp_remaining_keys_dic,
        ...
        try:
            ...
            shortest_path_weight = nx.path_weight(self.time_auxiliary_graph, shortest_path_nodes_list, "weight")
            if shortest_path_weight > 3.5*6E6:
        ...
        if efficiency_first_run and num_quantum_channels > threshold:
            ...
            return True, shortest_path_nodes_list, generated_key_rate, num_qkd_modules, num_quantum_channels, num_physical_hops, num_virtual_hops, num_qkp_hops

    def assign_shortest_path_auxiliary_graph(self, shortest_path_nodes_list, DG_tmp, assign_key_rate,
                                             qkp_remaining_keys_dic, channel_index, unused_channel_dic={},
                                             time_slot_current=-1):
        for node_index in range(len(shortest_path_nodes_list)-1):
            node1 = shortest_path_nodes_list[node_index]
            node2 = shortest_path_nodes_list[node_index+1]
            ...
            if node_int1 != node_int2:
                ...
                if quantum_node_flag1 and quantum_node_flag2:
                    ...
                    if time_slot_current != -1 and time_slot_current > time_slot_index1:
                        if channel_index in unused_channel_dic[node_int_left, node_int_right, time_slot_current]:
                            # todo: unused_channel_dic, we may also want to add it later
                            pass
                            # unused_channel_dic[node_int_left, node_int_right, time_slot_current].remove(channel_index)
                ...
                # for channel_index in range(0, self.num_total_channels):
                    ...
                    if selected_reach_index != -1:
                        ...
                        self.assign_shortest_path_auxiliary_graph(shortest_path_nodes_list=selected_shortest_path_nodes_list,
                                                                  ...
                                                                  unused_channel_dic=unused_channel_dic_tmp,
                                                                  time_slot_current=time_slot)
            ...
            # for time_slot in range(0, self.num_time_slots):
                ...
                # for channel_index in range(0, self.num_total_channels):
                    ...
                    for reach_index in range(0, len(reaches_list)):
                        ...
                        if shortest_path_flag:
                            # if reach_key_rate / self.num_time_slots >= request_key_rate_remaining:
                            if reach_key_rate >= request_key_rate_remaining:
                                served_current_channel = True
                                if num_qkd_modules < selected_consumed_module:
                                    selected_consumed_module = num_qkd_modules
                                    selected_reach_index = reach_index
                                ...
                                if generated_key_rate >= selected_generated_key_rate and num_qkd_modules < selected_consumed_module:
                                    ...
                                    selected_reach_index = reach_index
                                    selected_generated_key_rate = generated_key_rate
                                    selected_num_paths = math.ceil(request_key_rate_remaining / (reach_key_rate / self.num_time_slots))
                                    selected_shortest_path_nodes_list = shortest_path_nodes_list
                                    selected_num_physical_hops = num_physical_hops
                                    selected_num_virtual_hops = num_virtual_hops
                                    selected_num_qkp_hops = num_qkp_hops
